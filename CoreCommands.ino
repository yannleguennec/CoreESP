...
void CoreCommands::setup(void)
{
  ...
  CoreCommands::add( "help",    CoreCommands::help,   "Display help on commands (HELP <cmd>)" );
  CoreCommands::add( "info",    CoreCommands::info,   "Display info on chip" );
  CoreCommands::add( "restart", CoreCommands::restart, "Restarts the ESP" );
  CoreCommands::add( "reboot",  CoreCommands::reboot, "Reboots the ESP" );
...
void CoreCommands::help(String &res, char **block)
{
  ...
  {
    commandNo = CoreCommands::search( commandArg(1) );
    if (commandNo < commandNb)
      CoreCommands::display(commandNo);
  }
}
